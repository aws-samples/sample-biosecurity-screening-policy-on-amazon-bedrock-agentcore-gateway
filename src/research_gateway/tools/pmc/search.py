...
def fetch_pmc(pmc_ids: List[str]) -> List[ArticleDict]:
    ...
    try:
        ...
        except DefusedXmlException as xml_error:
            logger.warning("Rejected unsafe XML response: %s", xml_error.__class__.__name__)
            raise Exception("Unsafe XML response from PM") from xml_error
...
def _extract_article_data(article_element: Any) -> ArticleDict:
...
def _clean_article_for_output(article: ArticleDict) -> dict:
    """Normalize an internal article dict into a tidy output dict.

    Splits the authors string, coerces year to int when possible, and drops
    internal-only fields like ``text``, ``source``, and the raw ``references``
    list so we only surface useful metadata.
    """

    authors_str = article.get("authors") or ""
    authors = [name.strip() for name in authors_str.split(",") if name.strip()]

    year: Any = article.get("year")
    ...
    return {
        ...
        "reference_count": len(article.get("references", []) or []),
        "referenced_by_count": article.get("referenced_by_count", 0),
    }


def search_pmc_structured(
    query: str,
    max_search_result_count: int = 100,
    max_filtered_result_count: int = 10,
    rerank_by: Literal["references", None] = "references",
) -> dict:
    """Search PMC and return structured results (no free-text formatting).

    Mirrors :func:`search_pmc` but returns article metadata as JSON-friendly
    dicts so callers - notably MCP clients via AgentCore Gateway - can
    ...
    logger.info(f"Searching PMC (structured) for: {query}")
    base_url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils"
    ...
    try:
        ...
        search_response = httpx.post(
            f"{base_url}/esearch.fcgi", data=_get_api_key_params(search_params)
        )
        search_response.raise_for_status()
        search_data = search_response.json()
        id_list = search_data["esearchresult"]["idlist"]
        ...
        if rerank_by == "references":
            articles = _calculate_referenced_by_counts(articles)
            articles = _rank_by_citations(articles)

        total_found = len(articles)
        if max_filtered_result_count is not None:
            articles = articles[:max_filtered_result_count]
        ...
        return {
            ...
            "ranked_by": rerank_by,
            "articles": [_clean_article_for_output(a) for a in articles],
        }
        ...
        return {
            "status": "error",
            "message": (
                f"HTTP {http_error.response.status_code} from PMC: {http_error}"
            ),
        }
    except httpx.RequestError as request_error:
        logger.error(f"Request error during PMC search: {request_error}")
        ...
        return {
            "status": "error",
            "message": f"Unexpected PMC response format: missing {key_error}",
        }
    except Exception as unexpected_error:  # noqa: BLE001
        logger.error(f"Unexpected error in search_pmc_structured: {unexpected_error}")
